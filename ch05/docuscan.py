...
def drawROI(img, corners):
    ...
    disp = cv2.addWeighted(img, 0.3, cpy, 0.7, 0) # 가중치
...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: #  원 안에 마우스가 있을 경우 드래그
        for i in range(4):
            if cv2.norm(srcQuad[i] - (x, y)) < 25:
                dragSrc[i] = True
                ptOld = (x, y) # 원의 이동을 위한 좌표
                break
...
srcQuad = np.array([[30, 30], [30, h-30], [w-30, h-30], [w-30, 30]], np.float32) # 초기점의 좌표 반시계방향
